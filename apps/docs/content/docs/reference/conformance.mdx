...
`folderbase.root-reconstruction@0.1.0` has twelve implementation-neutral
...
This stable capability remains optional. Passing its suite proves the named
root-reconstruction contract; it does not widen Compatibility Contract v1.
...
`folderbase.folder-scope-evidence@0.1.0` has seven
implementation-neutral black-box cases. They prove exact arbitrary-folder
observation, idempotent journal replay, rename continuity, selected-folder and
root replacement refusal, nested-boundary isolation, unsafe invocation, and
escaping-symlink refusal. A passing report exits `0` with `passed: 7` and
`failed: 0`.

This stable capability remains optional. Passing its suite proves local
filesystem continuity evidence only; it does not create a Folder Scope ID,
share, grant, or Cloud authorization.
