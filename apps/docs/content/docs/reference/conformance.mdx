...
`folderbase.folder-scope-evidence@0.1.0` has eleven
...
cross-platform escaping-link and unsupported-node refusal, crash recovery, and
the closed aggregate journal namespace. A passing report exits `0` with
`passed: 11` and `failed: 0`.
