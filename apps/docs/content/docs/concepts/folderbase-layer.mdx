...
Optional human and agent entry points such as `FOLDERBASE.md`, `AGENTS.md`, and `CLAUDE.md` can explain the workspace without requiring an MCP server. They are ordinary guidance, not root authority; `.folderbase/manifest.json` establishes a native 0.5 boundary.
