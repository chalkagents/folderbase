...
const releaseAssetNames = [
  "folderbase-v0.5.0-aarch64-apple-darwin",
  "folderbase-v0.5.0-x86_64-apple-darwin",
  "folderbase-v0.5.0-aarch64-unknown-linux-gnu",
  "folderbase-v0.5.0-x86_64-unknown-linux-gnu",
];
...
test("a user invocation downloads, verifies, and runs the exact native CLI", async () => {
  ...
  const assetName = "folderbase-v0.5.0-aarch64-apple-darwin";
  ...
  try {
    ...
    assert.deepEqual(
      await readFile(join(cacheRoot, "v0.5.0", assetName)),
      nativeCli,
    );
  } finally {
...
test("a relative XDG cache path falls back to the home cache", async () => {
  ...
  const assetName = "folderbase-v0.5.0-aarch64-unknown-linux-gnu";
...
test("a release checksum matrix missing a non-host asset fails closed", async () => {
  const cacheRoot = await mkdtemp(join(tmpdir(), "folderbase-npx-test-"));
  const assetName = "folderbase-v0.5.0-aarch64-apple-darwin";
  const missingAssetName = "folderbase-v0.5.0-x86_64-unknown-linux-gnu";
  const nativeCli = Buffer.from("#!/bin/sh\nprintf 'must-not-run\\n'\n");
  const sha256 = createHash("sha256").update(nativeCli).digest("hex");
  const checksums = Buffer.from(
    `${releaseAssetNames
...
test("a streamed binary download larger than the bounded test limit fails closed", async () => {
  ...
  try {
    ...
    assert.equal(result.stdout, "");
    await assert.rejects(
      readFile(join(cacheRoot, "v0.5.0", assetName)),
      { code: "ENOENT" },
    );
  } finally {
    server.closeAllConnections();
...
test("invalid bounded-test byte limits fail closed before download", async () => {
  ...
  try {
    ...
    await assert.rejects(readFile(join(cacheRoot, "v0.5.0", "SHA256SUMS")), {
      code: "ENOENT",
    });
  } finally {
    await rm(cacheRoot, { recursive: true, force: true });
...
test("the bounded-test byte override is inert outside loopback test mode", async () => {
  const cacheRoot = await mkdtemp(join(tmpdir(), "folderbase-npx-test-"));
  const versionCache = join(cacheRoot, "v0.5.0");
  const assetName = "folderbase-v0.5.0-aarch64-apple-darwin";
...
test("a malformed closed checksum record fails before native execution", async () => {
  ...
  const checksums = Buffer.from(
    [
      `${sha256}  ${assetName}`,
      `${"1".repeat(64)}  folderbase-v0.5.0-x86_64-apple-darwin`,
      "this line is not a checksum",
      `${"2".repeat(64)}  folderbase-v0.5.0-aarch64-unknown-linux-gnu`,
      `${"3".repeat(64)}  folderbase-v0.5.0-x86_64-unknown-linux-gnu`,
      "",
    ].join("\n"),
  );
  let binaryRequests = 0;
...
test("an unexpected checksum entry fails before native execution", async () => {
  ...
  const checksums = Buffer.from(
    [
      `${sha256}  ${assetName}`,
      `${"4".repeat(64)}  folderbase-v0.5.0-x86_64-unknown-freebsd`,
      "",
    ].join("\n"),
  );
  let binaryRequests = 0;
