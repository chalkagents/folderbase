...
const releaseAssetNames = [
  "folderbase-v0.4.0-aarch64-apple-darwin",
  "folderbase-v0.4.0-x86_64-apple-darwin",
  "folderbase-v0.4.0-aarch64-unknown-linux-gnu",
  "folderbase-v0.4.0-x86_64-unknown-linux-gnu",
];
...
test("a relative XDG cache path falls back to the home cache", async () => {
  ...
  const assetName = "folderbase-v0.4.0-aarch64-unknown-linux-gnu";
...
test("a release checksum matrix missing a non-host asset fails closed", async () => {
  const cacheRoot = await mkdtemp(join(tmpdir(), "folderbase-npx-test-"));
  const assetName = "folderbase-v0.4.0-aarch64-apple-darwin";
  const missingAssetName = "folderbase-v0.4.0-x86_64-unknown-linux-gnu";
  const nativeCli = Buffer.from("#!/bin/sh\nprintf 'must-not-run\\n'\n");
  const sha256 = createHash("sha256").update(nativeCli).digest("hex");
  const checksums = Buffer.from(
    `${releaseAssetNames
...
test("a streamed binary download larger than the bounded test limit fails closed", async () => {
  ...
  try {
    ...
    assert.equal(result.stdout, "");
    await assert.rejects(
      readFile(join(cacheRoot, "v0.4.0", assetName)),
      { code: "ENOENT" },
    );
  } finally {
    server.closeAllConnections();
...
test("invalid bounded-test byte limits fail closed before download", async () => {
  ...
  try {
    ...
    await assert.rejects(readFile(join(cacheRoot, "v0.4.0", "SHA256SUMS")), {
      code: "ENOENT",
    });
  } finally {
    await rm(cacheRoot, { recursive: true, force: true });
