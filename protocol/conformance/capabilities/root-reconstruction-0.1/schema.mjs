...
function validate(value, inputSchema, root, path) {
  ...
  if (schema.enum) {
    assert.ok(
      schema.enum.some((item) => isDeepStrictEqual(value, item)),
      `${path} is not allowed`,
    );
  }
  if (schema.type) {
  ...
  if (typeof value === "string") {
    ...
    if (schema.pattern) assert.match(value, new RegExp(schema.pattern), `${path} is invalid`);
  }
  if (typeof value === "number") {
    if (schema.minimum !== undefined) {
      assert.ok(value >= schema.minimum, `${path} is below minimum`);
    }
    if (schema.maximum !== undefined) {
      assert.ok(value <= schema.maximum, `${path} is above maximum`);
    }
  }
}

function validateReferenceOrder(index) {
  if (!Array.isArray(index.references)) return;
  let previous;
  for (const [position, reference] of index.references.entries()) {
    const current = reference.object_version_id;
    if (previous !== undefined) {
      assert.ok(
        Buffer.compare(Buffer.from(previous, "utf8"), Buffer.from(current, "utf8")) < 0,
        `packageIndex.references[${position}].object_version_id is not in strict canonical order`,
      );
    }
    previous = current;
  }
