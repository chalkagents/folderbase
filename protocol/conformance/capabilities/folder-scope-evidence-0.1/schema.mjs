...
function validate(value, inputSchema, root, path) {
  ...
  if (schema.type) {
    assert.ok(matchesType(value, schema.type), `${path} has the wrong type`);
  }
...
export function assertFolderScopeEvidenceSchema(value, root, definition) {
  const schema = root.$defs?.[definition];
  assert.ok(schema, `unknown folder-scope-evidence schema definition ${definition}`);
