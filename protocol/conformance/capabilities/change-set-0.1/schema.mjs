...
function validate(value, inputSchema, root, path) {
  ...
  if (schema.allOf) for (const branch of schema.allOf) validate(value, branch, root, path);
  if (schema.oneOf) {
    const matches = schema.oneOf.filter((branch) => accepts(value, branch, root, path));
    assert.equal(matches.length, 1, `${path} must match exactly one branch`);
    return;
  }
  if (schema.not) assert.ok(!accepts(value, schema.not, root, path), `${path} is forbidden`);
...
export function assertChangeSetSchema(value, root, definition) {
  const schema = root.$defs?.[definition];
  assert.ok(schema, `unknown Change Set schema definition ${definition}`);
